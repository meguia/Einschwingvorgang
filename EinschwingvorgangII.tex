...
\begin{subequations} \label{eq_Ncoupled}
\begin{align} 
    \dot{\theta}_i &= \omega_i + K \left(\sin(\theta_{i-1}
    - \theta_i)+\sin(\theta_{i+1} - \theta_i)\right)  
\end{align}
\end{subequations}
...
To show another interesting aspect of coupled systems is the emergence of patterns and structures from the interaction, something that can be seen in Figure 8 but that will be much more evident if instead of phase oscillators we couple excitable systems. 
